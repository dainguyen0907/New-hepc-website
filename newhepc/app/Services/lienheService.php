...
//Vị trí: Trang liên hệ
    ...
    {
        if (session('contactTime') && session('contactTime') > time() - 900) {
            return [
        ...
        $param = $req->getPost();
        $data = [
            'ten' => trim($param['name_contact']),
            'sdt' => trim($param['number_contact']),
            'mail' => trim($param['email_contact']),
            'id_p' => $param['group_contact'],
            'tieude' => trim($param['heading_contact']),
            'noidung' => trim($param['content_contact']),
